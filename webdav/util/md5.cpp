...
void MD5::update(uint1 *input, uint4 input_length)
{
	...
	{ // ie. we have enough to fill the buffer
		...
		for (input_index = buffer_space; input_index + 63 < input_length; input_index
				+= 64)
	...
	memcpy(buffer + buffer_index, input + input_index, input_length
			- input_index);
}

// MD5 update for strings.
// like above, except that it works on strings
